...
from gz_public import region_search
...
@pytest.mark.login
@pytest.mark.ios
# @pytest.mark.parametrize("login_data", ['cn_test'], indirect=True)
def test_login(appium_service, ios_driver):
    driver = ios_driver
...
@pytest.mark.loginChangRegion
# @pytest.mark.parametrize("repeat_count", list(range(2)))
@pytest.mark.ios
@pytest.mark.parametrize("login_data", ['cn_test', 'us', 'eu'] * 200, indirect=True)
def test_login_change_region(appium_service, ios_driver, login_data, init_logout_ios):
    driver = ios_driver

    # 获取登录数据
    # login_data = LOGIN_DATA.get('cn_test').copy()
    logger.info(f"当前地区是：{login_data['region_name']}")
    ...
    with allure.step('step5: 选择地区'):
        if is_element_present(driver, AppiumBy.IOS_CLASS_CHAIN, login_data['region_type_text_field_class_chain']):
            logger.info(f"第五步：默认是{login_data['region_name']}的话，不需要点击进入地区选择列表")
            sleep(1)
        else:
            # 点击 选择国家/地区
            el = driver.find_element(by=AppiumBy.IOS_CLASS_CHAIN, value=REGION_TYPE_OTHER_CLASS_CHAIN)
            ...
            # 出现‘以后’就点击
            (el := element_until_appears(driver, AppiumBy.ACCESSIBILITY_ID, NOT_NOW, 3)) and el.click()
            logger.info(f"第五步：默认不是{login_data['region_name']}，"
                        f"需要在地区列表中滚动查找到{login_data['region_name']}")
            # 选择地区，例如，中国
            # el = scroll_find_element(driver, AppiumBy.IOS_PREDICATE, login_data['country_predicate'])
            # el.click()
            # 滚动查找速度较慢，使用搜索功能
            region_search(driver, login_data['region_name'])
    ...
    with (allure.step('step6: 点击 登录')):
        ...
        assert is_element_present(driver, AppiumBy.ACCESSIBILITY_ID, HOME_PAGE_ONLINE_SUPPORT) is not None, \
            "首页底部 ‘在线客服’标签 没有出现"
        assert is_element_present(driver, AppiumBy.ACCESSIBILITY_ID, login_data['assert_region_name']) is not None, \
            "大区没有出现"
...
def test_logout(appium_service, ios_driver):
    ...
    with allure.step('step4: 点击右上角的 X'):
        el = driver.find_element(by=AppiumBy.ACCESSIBILITY_ID, value=CLOSE_ICON_NAME)
        el.click()

        assert is_element_present(driver, AppiumBy.ACCESSIBILITY_ID, value=ICON_LOGO_NAME) is not None, \
            "没有回到启动页面"
